...
void get_medicines(char filename[])
{
    int i=0;
	FILE * fp;
	fp = fopen(filename, "r");
...
void request_delivery(char **city,int tcity)
{
    int i;
    int med_id;
    int quantity;
...
void menu(USER *db, int *users,char **city, int tcity)
{
	int sign = is_signed();
	printf("======================\n");
	if (sign)
	{
		...
		switch (ch)
		{
			case 1:
				//TO DO
				request_delivery(city,tcity);
				menu(db, users,city,tcity);
				break;
			case 2:
				// TO DO
				menu(db, users,city,tcity);
...
int main()
{
	...
	menu(users, userp,city_name, vertices);
